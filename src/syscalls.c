...
int tcasm_observer(xemem_segid_t seg) {
	int status;
	void *vaddr;
	struct xemem_addr addr;
	printf("ENTERING OBSERVER\n");
	printf("seg = %x\n",(unsigned int)seg);
	apid = xemem_get(seg, XEMEM_RDWR);
	if(apid == -1 ){
	...
	addr.apid = apid;
	addr.offset = 0;
	vaddr = xemem_attach(addr, 8192*sizeof(int), NULL);
	...
	printf("OBSERVING!\n");
	// so now I need to write the analytics.
}
...
int setup1_(int *nx, int *ny, int *nz, int *ng) {
	...
	printf("XPMEM MAKE\n");
	// TODO(npe) add segname
	seg1 = xemem_make(ptr, 8192*sizeof(int), "bob");
	if(seg1 <= 0){
...
int share_init_(int*iproc) {
	...
	printf("xpmem make\n");
	seg = xemem_make(cte, 8192*sizeof(int), "bob1");
	if(seg <= 0){
...
int allocate_(unsigned long *size, void **data)
{
